...
这是一个面向 Nintendo Switch 自制程序开发的自包含 AI Skill，适用于 libnx 与 devkitPro C/C++ 项目。它为 AI 编程助手提供带有明确版本的本地参考资料，让模型根据真实头文件编写和检查代码，不依赖猜测的 API 或某台电脑上的固定路径。
...
Switch 自制程序 API 很容易记错，而且在线资料未必与项目使用的版本一致。这个 Skill 把参考资料直接保存在仓库中，并要求 AI 编程助手在生成代码前核对内置头文件中的真实函数签名。
...
1. `SKILL.md` 告诉 AI 编程助手何时以及如何处理 Switch 自制程序任务。
...
| 组件 | 内置版本 | 用途 |
|---|---:|---|
| [libnx](https://github.com/switchbrew/libnx) | v4.12.0 官方签名标签 | 权威头文件、源码与代码风格规范 |
